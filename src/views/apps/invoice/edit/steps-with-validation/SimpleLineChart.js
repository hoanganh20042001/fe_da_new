...
const SimpleLineChart = ({ warning, stepper, configId }) => {
  const {
    handleSubmit
  } = useForm({})
  ...
  const handleStop = () => {
    ...
    axios.get(`${url}/experiment/stop-train/?id_paramsconfigs=${configId}`).then(
      res => {
        setCounter(100)
        setStop(true)
        setNext(true)
  ...
  useEffect(() => {
    ...
    if (counter < 1000 && statusTrain === 'training' && configId !== 0) {
      const interval = setInterval(() => {
        setCounter((prevCounter) => prevCounter + 1)
        const url = process.env.REACT_APP_API_URL
        axios.get(`${url}/experiment/get-new-traning-result/?id_paramsconfigs=${configId}&pre_result_index=0`
          , {
            headers: {
