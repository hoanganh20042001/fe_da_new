...
// ** Reactstrap Imports
import { Label, Row, Col, Button, Form, Input, FormFeedback, Badge, Table } from 'reactstrap'
...
const StepTest = ({ stepper, infoExp, changeInfo, info }) => {
  // ** Hooks
  const dispatch = useDispatch()
  const [statusTrain, setSatusTrain] = useState(0)
  ...
  const onSubmit = () => {
    ...
    if (infoModel.id !== 1 && info.configid !== 1) {
      ...
      <Form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className='d-flex justify-content-between'>
          ...
          <Button type='submit' color='primary' className='btn-next'>
            <span className='align-middle d-sm-inline-block d-none' disabled={disabled} >Kiểm tra</span>
          </Button>
